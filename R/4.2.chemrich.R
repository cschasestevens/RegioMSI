...
#' generated by msi_stat_anova() (default) or from a single
...
msi_stat_crich <- function(
  ...
  d2 <- d_ref
  cl1 <- cl_name
  if("label" %in% names(d2) == FALSE) { # nolint
    d2[["label"]] <- d2[[cl1]]
  }
  # Assign classes from reference
  d3 <- dplyr::left_join(
    d1,
    d2[, c(cmp_name, cl1, "label")],
    by = cmp_name
  )
            ...
            lapply(
              ...
              function(y) {
                ...
                ksdf2 <- setNames(data.frame(
                  ...
                  "Comparison.ID", "Comparison", "Cluster.ID", cl1, "p.value"
                ))
...
#' #   dfcr = d_enr[d_enr[["Comparison"]] == "comp1", ],
#' #   cl_name = "label.saturation"
#' # )
#'
#' @export
msi_plot_crich <- function(dfcr, cl_name) {
  df <- dfcr
  # Main Plot
  cr_plot <- ggplot2::ggplot(
    df,
    ggplot2::aes(
      x = .data[[cl_name]], # nolint
      y = -log10(FDR), # nolint
      fill = Ratio # nolint
    )
  ) +
